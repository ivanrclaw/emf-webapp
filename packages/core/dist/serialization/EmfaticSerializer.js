 ...
 */
export function serializeToEmfatic(pkg) {
    const lines = [];
...
function serializeAnnotation(ann, indent = '') {
...
function serializeClass(cls, idToName) {
    const lines = [];
...
function serializeAttribute(attr) {
    ...
    const modifiers = [];
    if (attr.derived)
        modifiers.push('derived');
    if (attr.transient)
        modifiers.push('transient');
    if (attr.changeable === false)
    ...
    const result = [...annotationLines, line];
    return result.join('\n');
}
function serializeReference(ref, idToName) {
    const annotationLines = [];
    if (ref.annotations) {
...
function serializeMultiplicity(lower, upper) {
    const lo = lower ?? 0;
    const up = upper ?? 1;
    if (lo === 0 && up === 1)
        return ''; // default, no brackets
    if (lo === 1 && up === 1)
        return '[1]';
    if (lo === 0 && up === -1)
        return '[*]';
 ...
 */
export function parseEmfatic(text) {
...
class EmfaticParser {
    ...
    pos = 0;
    pendingAnnotations = [];
    classifierNames = new Map(); // name → id
    constructor(lines) {
        this.lines = lines;
    }
    parse() {
     ...
     */
    collectClassifierNames() {
        for (const rawLine of this.lines) {
            const line = rawLine.trim();
            // Match class declarations
            const classMatch = line.match(/^(?:abstract\s+)?(?:class|interface)\s+(\w+)/);
            if (classMatch) {
    ...
    parseClass(firstLine) {
        ...
        const eSuperTypes = superTypeNames.map(sn => this.classifierNames.get(sn) || `cls_${sn}`);
        const cls = {
            id: `cls_${name}`,
            name,
            abstract: isAbstract,
    ...
    parseDataType(line) {
        ...
        const match = line.match(/^datatype\s+(\w+)\s*:\s*"((?:[^"\\]|\\.)*)"\s*;?$/);
        if (!match)
            return null;
        const name = match[1];
...
// Utility functions
// ═══════════════════════════════════════════════════════════════
function parseMultiplicity(mult) {
    if (!mult)
...
//# sourceMappingURL=EmfaticSerializer.js.map