...
export class OCLCompletionEngine {
     ...
     */
    complete(expression, cursorOffset, contextClassName) {
     ...
     */
    analyzeContext(expression, cursorOffset) {
     ...
     */
    completeDot(ctx, contextClassName) {
        const items = [];
     ...
     */
    completeArrow(ctx, contextClassName) {
        const items = [];
     ...
     */
    completeIdentifier(ctx, contextClassName) {
        const items = [];
    ...
    makeStdLibItem(op) {
        const params = op.params?.map((p) => `${p.name}: ${typeof p.type === 'string' ? p.type : typeToString(p.type)}`).join(', ') ?? '';
        const retStr = typeof op.returnType === 'string' ? op.returnType : typeToString(op.returnType);
    ...
    eTypeToOCL(eType) {
        switch (eType) {
            ...
            case 'EDouble':
            case 'EFloat':
            case 'EDoubleObject': return 'Real';
            case 'EBoolean':
...
//# sourceMappingURL=OCLCompletionEngine.js.map