...
function getEClassName(instance) {
    if (typeof instance.eClass === 'string')
        return instance.eClass;
    if (instance.eClass && typeof instance.eClass.name === 'string')
        return instance.eClass.name;
    return '';
}
function matchesContext(instance, context) {
...
// ── Batch Validation ─────────────────────────────────────────────────────
export function validateBatch(constraints, metamodel, modelInstances, evaluator, options) {
    const startTime = Date.now();
    const maxErrorsPerConstraint = options?.maxErrorsPerConstraint ?? Infinity;
    const timeoutMs = options?.timeoutMs ?? Infinity;
    const metamodelName = metamodel?.name ?? metamodel?.metamodelName ?? metamodel?.uri ?? 'Unknown';
    const results = [];
    ...
    for (const constraint of constraints) {
        const matchingInstances = modelInstances.filter((inst) => matchesContext(inst, constraint.context));
        const result = {
...
export function formatBatchReport(report) {
    ...
    if (report.results.length > 0) {
        ...
        for (const result of report.results) {
            const status = result.errorInstances > 0
                ? 'ERROR'
                : result.failedInstances > 0
                    ? 'FAIL'
                    : 'PASS';
            const statusIcon = status === 'PASS' ? '✓' : status === 'FAIL' ? '✗' : '⚠';
            lines.push(`  ${statusIcon} [${status}] ${result.context}::${result.constraintName || result.kind}`);
            lines.push(`    Instances: ${result.totalInstances} | Pass: ${result.passedInstances} | Fail: ${result.failedInstances} | Error: ${result.errorInstances}`);
...
//# sourceMappingURL=OCLBatchValidator.js.map