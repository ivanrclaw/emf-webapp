 ...
 */
export function generateEclipseProjectFiles(
  options: EclipseProjectOptions,
): EclipseProjectFiles {
  return {
...
function generateDotProject(options: EclipseProjectOptions): string {
...
function generateClasspath(options: EclipseProjectOptions): string {
...
function generateManifest(options: EclipseProjectOptions): string {
  const emfVersion = options.emfVersion || '2.38.0';
  const vendor = options.vendor || 'emf-webapp';

  const requiredBundles: string[] = [
...
function generatePluginXml(options: EclipseProjectOptions): string {
...
function generateBuildProperties(options: EclipseProjectOptions): string {
  const lines: string[] = [];
