 ...
 */
export function parseCompleteOCL(oclText: string): CompleteOCLParseResult {
  const result: CompleteOCLParseResult = {
    packageName: '',
    imports: [],
    constraints: [],
  };
  ...
  let currentContext = '';
  let currentSeverity: 'error' | 'warning' | 'info' = 'error';
  ...
  function flushConstraint(): void {
    if (currentInvName && currentExpression.trim()) {
      const constraint: OCLConstraintInput = {
        name: currentInvName,
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (severityMatch) {
      ...
      currentSeverity = severityMatch[1].toLowerCase() as 'error' | 'warning' | 'info';
      continue;
    }
 ...
 */
function isContinuationToken(trimmed: string): boolean {
