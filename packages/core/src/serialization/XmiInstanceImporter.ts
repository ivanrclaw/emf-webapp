...
export interface XmiInstance {
  eClass: string;  // class name from metamodel
  attributes: Record<string, any>;  // attribute values
  references: Record<string, XmiInstanceRef[]>;  // reference values
  children: Record<string, XmiInstance[]>;  // containment children
}

export interface XmiInstanceRef {
  type: 'internal' | 'external';
  path: string;  // fragment path like '#//@authors.0'
}
...
export interface XmiInstanceDocument {
  ...
  nsURI: string;
  nsPrefix: string;
  allInstances: XmiInstance[];  // flat list of all instances
}
 ...
 */
export function importXmiInstance(
  xml: string,
  ...
  const allInstances: XmiInstance[] = [];
...
function parseElement(
  xml: string,
  eClassName: string,
  classMap: Map<string, SerializableEClass>,
  metamodel: SerializableEPackage,
  allInstances: XmiInstance[]
): XmiInstance {
  const eClass = classMap.get(eClassName);

  const instance: XmiInstance = {
    eClass: eClassName,
    attributes: {},
    references: {},
    children: {},
    ...
    if (ref.containment) {
      ...
      if (inlineRef && !instance.references[ref.name]) {
        instance.references[ref.name] = inlineRef.split(/\s+/).filter(Boolean).map((path) => ({
          type: (path.startsWith('#') ? 'internal' : 'external') as 'internal' | 'external',
          path,
        }));
      }
    }
 ...
 */
export function exportXmiInstance(
 ...
 */
function buildAttributeStrings(
  instance: XmiInstance,
  classMap: Map<string, SerializableEClass>
): string[] {
  const result: string[] = [];
 ...
 */
function hasContent(instance: XmiInstance): boolean {
 ...
 */
export function generateFragmentPath(
  document: XmiInstanceDocument,
  target: XmiInstance
): string {
...
function findPath(
  ...
  if (current === target) return prefix || '/';
...
export function resolveFragmentPath(
  ...
  let current: XmiInstance = document.root;
 ...
 */
function buildClassMap(metamodel: SerializableEPackage): Map<string, SerializableEClass> {
  const map = new Map<string, SerializableEClass>();
  for (const classifier of metamodel.eClassifiers) {
    if ('eAttributes' in classifier) {
      map.set(classifier.name, classifier as SerializableEClass);
    }
  }
  return map;
}
 ...
 */
function convertAttributeValue(raw: string, eType: string): any {
  switch (eType) {
    case 'EInt':
    case 'EInteger':
    case 'ELong':
 ...
 */
function extractChildBlocks(xml: string, tagName: string): string[] {
  const results: string[] = [];

  // Find the end of the root opening tag to only search within its body
  const bodyStart = findBodyStart(xml);
  if (bodyStart === -1) return results; // self-closing root

  const bodyEnd = xml.lastIndexOf('</');
  if (bodyEnd === -1) return results;
 ...
 */
function findBodyStart(xml: string): number {
  let i = 0;
