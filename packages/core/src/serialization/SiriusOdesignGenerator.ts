...
// ═══════════════════════════════════════════════════════════════
// Types (mirrors frontend spec-diagram/types.ts)
// ═══════════════════════════════════════════════════════════════
...
const CHILDREN_PRESENTATION_MAP: Record<string, string> = {
  'FreeForm': 'FreeForm',
  'List': 'List',
 ...
 */
export function generateOdesign(
  spec: OdesignViewpointSpec,
  options: OdesignOptions,
): string {
  const lines: string[] = [];
...
function emitNodeMapping(
  lines: string[],
  nm: OdesignNodeMapping,
  options: OdesignOptions,
  depth: number,
): void {
...
function emitContainerMapping(
  lines: string[],
  cm: OdesignContainerMapping,
  options: OdesignOptions,
  depth: number,
): void {
  const ind = ' '.repeat(depth);
...
function emitEdgeMapping(
  lines: string[],
  em: OdesignEdgeMapping,
  options: OdesignOptions,
  depth: number,
): void {
  const ind = ' '.repeat(depth);
...
function emitToolSection(
  lines: string[],
  ts: OdesignToolSection,
  options: OdesignOptions,
  depth: number,
): void {
  const ind = ' '.repeat(depth);
 ...
 */
function toAQL(expr: string): string {
  if (!expr) return '';
  if (expr.startsWith('aql:')) return expr;
  if (expr.startsWith('feature:')) return expr;
  if (expr.startsWith('service:')) return expr;
 ...
 */
function qualifyDomainClass(className: string, nsURI: string): string {
  if (!className) return '';
  if (className.includes('::')) return className; // Already qualified
 ...
 */
function getStyleType(shape: string): string {
  switch (shape) {
