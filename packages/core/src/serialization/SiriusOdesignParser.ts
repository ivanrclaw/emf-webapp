...
function stripAqlPrefix(expr: string | undefined): string {
  if (!expr) return '';
  if (expr.startsWith('aql:')) return expr.substring(4);
  return expr;
}

/** Extract class name from qualified domain class (nsURI::ClassName → ClassName) */
function extractClassName(qualifiedName: string | undefined): string {
...
function convertDecoration(decoration: string | undefined): string {
...
function convertLineStyle(style: string | undefined): string {
...
function convertRoutingStyle(routing: string | undefined): string {
  if (!routing) return 'straight';
...
function extractColor(el: XmlElement, colorAttrName: string): string {
...
function resolveColor(el: XmlElement, attrName: string, defaultColor: string = '#000000'): string {
...
function parseNodeStyle(styleEl: XmlElement): NodeStyle {
  ...
  if (styleEl.attrs['borderSizeComputationExpression']) {
    ...
    if (!isNaN(num)) style.borderSize = num;
  }
...
function parseEdgeStyle(styleEl: XmlElement): EdgeStyle {
  ...
  if (styleEl.attrs['sizeComputationExpression']) {
    ...
    if (!isNaN(num)) style.lineWidth = num;
  }
...
function parseNodeMapping(el: XmlElement): ParsedNodeMapping {
...
function parseContainerMapping(el: XmlElement): ParsedContainerMapping {
...
function parseEdgeMapping(el: XmlElement): ParsedEdgeMapping {
  const id = el.attrs['name'] || '';
  const useDomainElement = el.attrs['useDomainElement'] === 'true';
  const type: 'relation-based' | 'element-based' = useDomainElement ? 'element-based' : 'relation-based';
...
function parseTool(el: XmlElement): ParsedTool {
...
function parseToolSection(el: XmlElement): ParsedToolSection {
  const label = el.attrs['name'] || el.attrs['label'] || '';
...
function parseLayer(el: XmlElement): ParsedLayer {
  const name = el.attrs['name'] || 'Default';
  const nodeMappings: ParsedNodeMapping[] = [];
  const containerMappings: ParsedContainerMapping[] = [];
  const edgeMappings: ParsedEdgeMapping[] = [];
  const toolSections: ParsedToolSection[] = [];
...
function parseDiagramDescription(el: XmlElement): {
  diagram: ParsedViewpoint['diagram'];
  defaultLayer: ParsedLayer;
  additionalLayers: ParsedLayer[];
} {
  ...
  const additionalLayers: ParsedLayer[] = [];
 ...
 */
export function parseOdesign(xml: string): OdesignParseResult {
