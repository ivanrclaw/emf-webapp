 ...
 */
export function generateAcceleoModule(
  templates: CodeTemplateInput[],
  options: AcceleoModuleOptions,
): string {
  const lines: string[] = [];
 ...
 */
export function generateAcceleoSingleTemplate(
  template: CodeTemplateInput,
  options: AcceleoModuleOptions,
): string {
 ...
 *   [if (cond)]          → conditional
 */
function convertBodyToAcceleo(body: string, argName: string): string {
  let result = body;
 ...
 */
function convertExpressionToAcceleo(expr: string, argName: string): string {
  let result = expr;
