...
function primitivConformsTo(a: string, b: string): boolean {
  if (a === b) return true;
  // UnlimitedNatural → Integer → Real
  if (a === 'UnlimitedNatural' && (b === 'Integer' || b === 'Real')) return true;
  if (a === 'Integer' && b === 'Real') return true;
  return false;
}

// ── Collection Conformance ──────────────────────────────────────────

function collectionConformsTo(
  a: CollectionType,
  b: CollectionType,
  hierarchy?: ClassHierarchy,
): boolean {
...
function tupleConformsTo(
  ...
  for (const bPart of b.parts) {
    ...
    if (!conformsTo(aPart.type, bPart.type, hierarchy)) return false;
  }
 ...
 */
export function commonSupertype(
  a: OCLType,
  b: OCLType,
  hierarchy?: ClassHierarchy,
): OCLType {
  ...
  if (conformsTo(a, b, hierarchy)) return b;
  if (conformsTo(b, a, hierarchy)) return a;
  ...
  if (a.kind === 'primitive' && b.kind === 'primitive') {
    ...
    if (
      (a.name === 'UnlimitedNatural' && b.name === 'Integer') ||
    ...
    if (
      (a.name === 'UnlimitedNatural' && b.name === 'Real') ||
