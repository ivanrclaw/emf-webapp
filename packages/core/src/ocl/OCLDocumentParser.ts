...
export class OCLDocumentParser {
  ...
  private pos = 0;
  private errors: OCLDocumentError[] = [];
  private readonly exprParser = new OCLParser();
  ...
  parse(input: string): OCLDocumentParseResult {
    ...
    const declarations: (PackageDeclNode | ContextDeclNode)[] = [];
  ...
  private parsePackage(): PackageDeclNode {
    this.expect(TokenType.PACKAGE);
    const name = this.parseQualifiedName();
    const contexts: ContextDeclNode[] = [];
  ...
  private parseContext(): ContextDeclNode {
  ...
  private parseClassifierContext(className: string): ContextDeclNode {
    const constraints = this.parseConstraints();
    return { type: 'context', kind: 'classifier', className, constraints };
  }

  private parseOperationContext(className: string, opName: string): ContextDeclNode {
  ...
  private parsePropertyContext(className: string, propName: string): ContextDeclNode {
    let returnType: string | undefined;
    if (this.check(TokenType.COLON)) {
      this.advance();
      returnType = this.parseTypeName();
  ...
  private parseConstraints(): ConstraintNode[] {
  ...
  private parsePrecondition(): PreConditionNode {
  ...
  private parsePostcondition(): PostConditionNode {
  ...
  private parseDerive(): DeriveNode {
  ...
  private parseExpression(): ASTNode {
    ...
    while (!this.isAtEnd()) {
      ...
      if (tok.type === TokenType.LPAREN || tok.type === TokenType.LBRACE) depth++;
      if (tok.type === TokenType.RPAREN || tok.type === TokenType.RBRACE) depth--;
    ...
    if (exprTokens.length === 0) {
      ...
      return { type: 'literal', value: null, valueType: 'null' } as ASTNode;
    }
  ...
  private parseParam(): OperationParam {
    const name = this.expectIdentifierOrKeyword();
    this.expect(TokenType.COLON);
  ...
  private addError(message: string, position: number, recoverable: boolean): void {
