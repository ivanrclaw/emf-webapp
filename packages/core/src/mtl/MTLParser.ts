...
export class MTLParser {
  ...
  private parseTemplateList(): MTLTemplate[] {
    const templates: MTLTemplate[] = [];
    while (this.pos < this.tokens.length) {
      const token = this.peek()!;
      if (token.startsWith('[') && token.endsWith(']')) {
        const tag = this.parseTag(token);
  ...
  private parseTemplate(openToken: string): MTLTemplate | null {
    ...
    const filteredBody = body.filter(
      n => !(n.type === 'comment' && n.text.trim() === '@main') && n.type !== 'template'
    );
   ...
   */
  private parseNodes(endTags: string[] = []): MTLNode[] {
    const nodes: MTLNode[] = [];
  ...
  private parseTag(token: string): ParsedTag | null {
    ...
    const templateMatch = content.match(
      /^template\s+public\s+(\w+)\s*\(\s*(\w+)\s*:\s*(\w+(?:\.\w+)*)\s*\)\s*$/
    );
    if (templateMatch) {
      return {
