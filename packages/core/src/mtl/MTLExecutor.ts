...
export class MTLExecutor {
  ...
  private queryMemo: Map<string, any> = new Map();
  /** Module metadata for visibility enforcement */
  private moduleRegistry: Map<string, MTLModule> = new Map();
  /** Maps template/query name → owning module name */
  private symbolOwner: Map<string, string> = new Map();
  /** Maps template name → parent (overridden) function for super calls */
  private superMap: Map<string, (...args: any[]) => string> = new Map();
   ...
   */
  loadProperties(files: Array<{ name: string; content: string }>): void {
   ...
   */
  execute(nodes: MTLNode[], model: EObject, options?: { enableLogging?: boolean }): MTLExecutionResult {
    try {
      this.traceOutput = [];
      this.fileManager = new MTLFileManager();
      ...
      const context: Record<string, any> = {
        ...
        getProperty: (...args: any[]) => this.resolveProperty(args),
      };
  ...
  private resolveQueryCache(nodes: MTLNode[]): Record<string, (...args: any[]) => any> {
    ...
    for (const n of nodes) {
      ...
      if (n.type === 'query') {
        cache[n.name] = (...args: any[]) => {
          ...
          if (this.queryMemo.has(cacheKey)) return this.queryMemo.get(cacheKey);
          const qCtx: Record<string, any> = { ...cache };
    ...
    const processModule = (mod: MTLModule) => {
      if (processed.has(mod.name)) return;
      // Process parent first if extends
      if (mod.extends) {
        const parent = modules.find(m => m.name === mod.extends);
        if (parent) processModule(parent);
      }
      ...
      for (const t of mod.templates) {
        ...
        cache[t.name] = (...args: any[]) => {
          this.checkVisibility(t.name, t.visibility, mod.name);
          // Check guard
          if (t.guard) {
            const guardCtx: Record<string, any> = { ...cache, self: args[0] };
          ...
          const tCtx: Record<string, any> = { ...cache, self: args[0] };
    ...
    for (const m of modules) processModule(m);
    return cache;
  }
   ...
   */
  private checkVisibility(symbolName: string, visibility: Visibility, ownerModule: string): void {
    if (visibility === 'public') return; // always accessible
    if (!this.currentModule) return; // top-level call (from @main), allow
    if (this.currentModule === ownerModule) return; // same module, allow
    ...
    if (visibility === 'protected') {
      ...
      while (mod?.extends) {
        if (mod.extends === ownerModule) return; // allowed via extends chain
   ...
   */
  private summarizeArg(arg: any): string {
   ...
   */
  private applyPostTreatment(output: string, postExpr: string): string {
    // Common post expressions: trim(), replaceAll('x', 'y')
    const ctx: Record<string, any> = { self: output };
   ...
   */
  private resolveProperty(args: any[]): string {
  ...
  private executeArrowOp(source: any, operation: string, rawArgs: string, context: Record<string, any>): any {
    ...
    switch (operation) {
      ...
      case 'excludesAll': return Array.isArray(arg0) && arg0.every((a: any) => !source.includes(a));
      ...
      case 'asOrderedSet': {
        ...
        for (const item of source) { if (!seen.has(item)) { seen.add(item); unique.push(item); } }
        return unique;
      }
      case 'asSet': {
        const unique2: any[] = [];
        const seen2 = new Set<any>();
        for (const item of source) { if (!seen2.has(item)) { seen2.add(item); unique2.push(item); } }
        return unique2;
      }
      case 'asBag': return [...source];
      case 'sum': return source.reduce((a: number, b: any) => a + (Number(b) || 0), 0);
      case 'indexOf': return source.indexOf(arg0);
      case 'including': return source.includes(arg0) ? source : [...source, arg0];
      case 'excluding': return source.filter((x: any) => x !== arg0);
      case 'union': return [...source, ...(Array.isArray(arg0) ? arg0 : [])];
      case 'intersection': return source.filter((x: any) => arg0?.includes?.(x));
      ...
      case 'count': return source.filter((x: any) => x === arg0).length;
      case 'product': return source.reduce((a: number, b: any) => a * (Number(b) || 1), 1);
        ...
        while (pending.length > 0) {
          ...
          for (const item of pending) {
            ...
            if (Array.isArray(val)) {
              for (const v of val) { if (!visited.has(v)) next.push(v); }
  ...
  private callMethod(obj: any, method: string, rawArgs: string, context: Record<string, any>): any {
    ...
    switch (method) {
      ...
      case 'current': {
        ...
        if (this.iterationStack.length === 0) return obj;
        const depth = args.length > 0 ? Number(args[0]) : 0;
        const stackIdx = this.iterationStack.length - 1 - depth;
        if (stackIdx < 0 || stackIdx >= this.iterationStack.length) return undefined;
        const frame = this.iterationStack[stackIdx];
        return frame.collection[frame.index];
      }
    }
    ...
    if (typeof obj === 'string') {
      switch (method) {
        ...
        case 'replaceAll': case 'substituteAll': return obj.replace(new RegExp(String(args[0]), 'g'), String(args[1] ?? ''));
        case 'toInteger': return parseInt(obj, 10);
        case 'tokenize': return obj.split(String(args[0] ?? '\\\\s+'));
        case 'indexOf': case 'index': return obj.indexOf(String(args[0]));
        case 'first': return obj.charAt(0);
        case 'last': return obj.charAt(obj.length - 1);
        case 'lastIndex': case 'lastIndexOf': return obj.lastIndexOf(String(args[0]));
        case 'contains': return obj.includes(String(args[0]));
        case 'replace': case 'substitute': return obj.replace(String(args[0]), String(args[1] ?? ''));
    ...
    if (Array.isArray(obj)) {
      switch (method) {
        ...
        case 'asOrderedSet': {
          const unique2: any[] = [];
          const seen2 = new Set<any>();
          for (const item of obj) { if (!seen2.has(item)) { seen2.add(item); unique2.push(item); } }
          return unique2;
        }
        case 'asBag': return [...obj];
        case 'indexOf': return obj.indexOf(args[0]);
        case 'insertAt': { const idx = Number(args[0]); return [...obj.slice(0, idx), args[1], ...obj.slice(idx)]; }
        case 'sep': return obj.join(String(args[0] ?? ', '));
        case 'excludesAll': return Array.isArray(args[0]) && args[0].every((a: any) => !obj.includes(a));
        case 'includesAll': return Array.isArray(args[0]) && args[0].every((a: any) => obj.includes(a));
        case 'collect': return obj.map((x: any) => this.resolveOnObject(x, String(args[0])));
        case 'sortedBy': return [...obj].sort((a: any, b: any) => { const va = this.resolveOnObject(a, String(args[0])); const vb = this.resolveOnObject(b, String(args[0])); return va < vb ? -1 : va > vb ? 1 : 0; });
      }
    }
    ...
    if (this.isEObject(obj)) {
      switch (method) {
        ...
        case 'eContents': {
          ...
          const refs = eClass.eAllReferences || eClass.eAllStructuralFeatures?.filter?.((f: any) => f.eType?.name !== 'EDataType') || [];
          const result: any[] = [];
          for (const ref of refs) { const val = obj.eGet(ref); if (Array.isArray(val)) result.push(...val); else if (val) result.push(val); }
          return result;
        }
        case 'oclIsKindOf': { const typeName = String(args[0]); const eClass = obj.eClass(); return eClass.name === typeName; }
        case 'oclIsTypeOf': { const typeName = String(args[0]); const eClass = obj.eClass(); return eClass.name === typeName; }
        case 'oclAsType': return obj;
        case 'oclIsUndefined': return false;
        case 'ancestors': {
          const result: any[] = [];
          let cur = obj.eContainer();
          while (cur) { result.push(cur); cur = cur.eContainer?.(); }
          return result;
        }
        case 'siblings': {
          ...
          return Array.isArray(siblings) ? siblings.filter((s: any) => s !== obj) : [];
        }
      }
    }
   ...
   */
  private collectAllInstances(typeName: string): any[] {
    if (!this.rootModel) return [];
    const result: any[] = [];
    const visited = new Set<any>();
    const walk = (o: any) => {
      if (!this.isEObject(o) || visited.has(o)) return;
      visited.add(o);
  ...
  private findLastArrowOutsideParens(str: string): number {
    let depth = 0;
    let lastIdx = -1;
  ...
  private findPipeOutsideParens(str: string): number {
    let depth = 0;
    for (let i = 0; i < str.length; i++) {
      if (str[i] === '(' || str[i] === '[') depth++;
      else if (str[i] === ')' || str[i] === ']') depth--;
